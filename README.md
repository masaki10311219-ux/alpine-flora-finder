...
## 今後の拡張予定（未着手・設計方針のみ決定済み）

### ユーザーによる写真投稿（Googleログイン）

「山で見つけた花」としてユーザーが写真を投稿できる機能。着手はまだだが、方針は以下で決定済み：

- 認証・保存基盤には **Supabase**（Auth + Storage + Postgres）を想定。ブラウザから直接呼び出せるため、サイト本体は無料静的ホスティングのまま維持できる
- **投稿は承認制にせず即時公開**する
- その代わり、**投稿に対して他のユーザーがコメント・報告できる機能**を設け、不適切な投稿はコミュニティベースで検知できるようにする（事後対応型のモデレーション）
- 実装時に必要な追加作業：Google Cloud ConsoleでのOAuthクライアント作成（ユーザー側对応）、Supabaseプロジェクトの作成、DBスキーマ設計（投稿・コメント・通報テーブル）

### 植物データのさらなる拡充

現在51種。網羅性を高めるため、種の追加は継続的なタスクとして残っている。
